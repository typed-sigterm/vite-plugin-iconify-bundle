...
describe('vite-plugin-iconify-offline', () => {
  it('should find icons in transform hook', async () => {
    const plugin = iconifyOffline() as any;
    ...
    expect(result).toContain('addCollection');
    expect(result).toContain('prefix: "lucide"');
  });
  ...
  it('should find icons in files globs', async () => {
    ...
    const plugin = iconifyOffline({ files: [tempFile] }) as any;
  ...
  it('should handle additional icons', async () => {
    const plugin = iconifyOffline({ additional: ['lucide:check-square'] }) as any;
